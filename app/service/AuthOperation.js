...
class AuthOperation {
  constructor() {
    ...
    this.selectedPlugin = storageService.get('selectedPlugin') || 'GEM';
  ...
  login(subscribeOption, selectedPlugin) {
    this.selectedPlugin = selectedPlugin.toUpperCase();
    storageService.set('selectedPlugin', this.selectedPlugin);
  ...
  getUserProfile() {
    ...
    var request = {
      token: storageService.get('oasis_id_token'),
      origin_app: this.selectedPlugin,
  ...
  parseHash() {
    var token = storageService.get('oasis_id_token');
    if (token !== null && !this.isTokenExpired(token)) {
      this.show_logged_in(token);
    } else {
      var result = this.auth0.parseHash(window.location.hash);
      if (result && result.idToken) {
        storageService.set('oasis_id_token', result.idToken);
